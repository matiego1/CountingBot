    ...
    public void onSlashCommandInteraction(@NotNull SlashCommandInteraction event) {
        ...
        Utils.async(() -> {
            if (chn == null) {
                ...
                Logs.info(DiscordUtils.getAsTag(user) + " unblocked " + success + " counting channel(s) out of " + channels.size() + ".");
            ...
            if (data.unblock(jda)) {
                Logs.info(DiscordUtils.getAsTag(user) + " unblocked counting channel. (ID: `" + data.getChannelId() + "`; Guild ID: `" + data.getGuildId() + "`; Channel type: `" + data.getType() + "`)");
